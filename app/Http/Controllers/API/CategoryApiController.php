...
class CategoryApiController extends Controller
{
    public function list(Request $request)
    {
         ...
         if ($searchValue != '') {
             ...
             $searchQuery = " Categoies.id like '%" . $searchValue . "%' or name like '%" . $searchValue . "%' ";
         }
         ## Total number of records without filtering
         $totalRecords = Category::all()->count();
 
         ## Total number of record with filtering
         if ($searchQuery) {
             $totalRecordwithFilterQuery =  Category::offset($start)->limit($rowperpage);
             $totalRecordwithFilterQuery->whereRaw($searchQuery);
             $totalRecordwithFilter = $totalRecordwithFilterQuery->count();
         } else {
             $totalRecordwithFilter =  Category::all()->count();
         }
 
         ## Fetch records
         $recordsQuery = Category::offset($start)->limit($rowperpage)->orderBy($columnName, $columnSortOrder);
         $recordsQuery->select('Categories.*');
         ...
         foreach ($records as $record) {
             ...
             <a href="' . url('/admin/category/' . $record->id) . '" type="button" class="btn btn-outline-secondary">
             <i class=" mdi mdi-table-edit icon-md text-warning"></i>
             </a>
             <a href="' . url('/admin/category/status/' . $record->id ) . '"  class="btn btn-outline-secondary">
